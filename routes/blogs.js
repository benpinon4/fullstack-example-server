...
router.get('/fromDB', async  function(req, res, next) {
  const getAllBlogsMongoDB = await db().collection('blogs').find({}).toArray()
  
  res.json({
  sucess: true,
  allblogs: getAllBlogsMongoDB
})

})

router.get('/getOne', async function(req, res, next) {
  const getOneBlog = await db().collection('blogs').findOne()
...
router.get('/getByID/:id', async function(req, res, next) {
  ...
  const findOneIDParams = req.params.id
  console.log(findOneIDParams)
  const idtoUse = "976604e5-b10f-47fb-9191-dcb23df24277"
  const getOneBlogByID = await db().collection('blogs').findOne({
    id: findOneIDParams
  })
  
  res.json({
    sucess:true,
    oneBlogByID: getOneBlogByID
  })
})

// router.get('/getByIDDD/:idDrop', async function(req, res, next) {
  
//   const findOneIDParams = req.params.idDrop
//   console.log(findOneIDParams)
 
//   const getOneBlogByIDDD = await db().collection('blogs').findOne({
//     id: findOneIDParams
//   })

//   res.json({
//     sucess:true,
//     oneBlogByIDDD: getOneBlogByIDDD
//   })
// })
